...
Audit readiness findings are reconciled with quality-capability detection at the audit layer: audit computes quality detection first and suppresses the "Top-level test directory not detected." readiness info whenever `tests` is detected through another supported mechanism, including bounded Git-aware Go `*_test.go` discovery in the shared quality detector. The Project Map keeps raw inventory facts, and Node/Python behavior is unchanged (Task 0122).
