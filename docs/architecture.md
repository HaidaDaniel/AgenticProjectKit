...
Claim baselines use a parallel append-only `.agentic/task-baselines.jsonl` store. A baseline captures HEAD, dirty-file fingerprints, task path, owner, and explicit bookkeeping exclusions. Scope attribution preserves the earliest task baseline and recomputes bounded Git lineage from existing baseline, task, evidence, and run-log records. Git path lists are NUL-delimited and preserved exactly; invalid UTF-8 and literal backslash paths fail closed. A successful lineage result is bound to its evaluated HEAD, which scope and candidate capture recheck along with dirty paths overlapping excluded commits. It excludes a commit only when it is a direct child of another task's baseline and that task's successful `done` evidence binds the exact commit, candidate, baseline, and gate evidence set; the other task contract must also be unchanged from the Git parent through the candidate tree. A narrow task-file-only terminal bookkeeping child is recognized separately. Release/reclaim intervals require every intervening commit to have that proof and preserve identical dirty fingerprints. Same-file overlap, anonymous commits during release, dirty changes, missing evidence, merges, nonlinear history, and over-limit histories fail closed. APK stores no parallel ownership ledger and does not infer ownership from commit author or text.
...
`src/core/resources/detect.ts` builds a deterministic, read-only inventory from declared resources, local harness markers, and the shared quality-capability detector, with a stable fingerprint and no secret access. `src/core/execution/calibrate.ts` emits a bounded `apk-calibration-v1` planner package, validates an external recommendation deterministically (unknown IDs, capacity, availability, profile, assurance, secret-shaped values), and applies only the generated `executionCalibration` field on explicit request, preserving user overrides. Saved calibration is advisory and stale when its inventory fingerprint changes; there is no provider SDK, model runtime, or second config authority.
...
Human/operator decisions for review-budget exhaustion are a first-class append-only `human-decision` evidence type in the existing single evidence store. Records carry a bounded decision vocabulary (`accept-current`, `grant-review-passes`, `changes-required`), an `actor` distinct from the recording agent, and an honest `operator-asserted` trust model. The completion gate consumes structured review/budget/decision state: `accept-current` resolves only the `review-budget-exhausted` condition while deterministic verification, scope, dependency, evidence, and assurance blockers remain typed hard blockers, and budget extension is additive with a +2-per-candidate cap. No parallel gate, generic bypass, or automatic decision synthesis exists (Task 0119, ADR-0059).

Audit readiness findings are reconciled with quality-capability detection at the audit layer: audit computes quality detection first and suppresses the "Top-level test directory not detected." readiness info whenever `tests` is detected through another supported mechanism, including canonical Git-inventory Go `*_test.go` discovery (tracked plus non-ignored untracked, `.gitignore`-aware) in the shared quality detector. The Project Map keeps raw inventory facts, and Node/Python behavior is unchanged (Tasks 0122, 0135).
