...
`.github/workflows/quality.yml` is AgenticProjectKit-only repository tooling. Its single bounded clean-checkout job pins the supported Node.js/pnpm versions, installs with `--frozen-lockfile`, asserts `GITHUB_SHA == git rev-parse HEAD`, runs the local quality/release surface plus read-only APK lint/sync checks, and runs report-writing audit on the disposable runner. A final tracked-drift check makes audit or generated-output mutation visible. CI proves reproducibility for checked-out SHA; it never substitutes for candidate-bound APK verification, independent review, gate, or Task 0075 frozen-release evidence.
