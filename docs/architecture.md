...
Claim baselines use a parallel append-only `.agentic/task-baselines.jsonl` store. A baseline captures HEAD, dirty-file fingerprints, task path, owner, and explicit bookkeeping exclusions. Scope attribution preserves the earliest task baseline and recomputes bounded Git lineage from existing baseline, task, evidence, and run-log records. It excludes a commit only when it is a direct child of another task's baseline and that task's successful `done` evidence binds the exact commit, candidate, baseline, and gate evidence set; a narrow task-file-only terminal bookkeeping child is recognized separately. Release/reclaim intervals require every intervening commit to have that proof and preserve identical dirty fingerprints. Same-file overlap, anonymous commits during release, dirty changes, missing evidence, merges, nonlinear history, and over-limit histories fail closed. APK stores no parallel ownership ledger and does not infer ownership from commit author or text.
