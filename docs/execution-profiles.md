...
Precedence is deterministic and implemented exactly in `apk execution explain`: built-in default profile -> authored `executionProfile` -> current validated calibration recommendation -> authored `executionOverrides` -> explicit CLI override -> canonical safety/policy validation. A profile is chosen as CLI `--profile` > current calibration profile > authored `executionProfile` > default. A route target is chosen as authored/CLI override > current calibration route > deterministic resolver. A calibrated worker route applies only when the named worker passes capability/availability/capacity/profile checks. Calibration sentinels are explicit: a current `wait` produces a calibration-sourced `wait`/`wait` route, a current `needs-human` produces a calibration-sourced `needs-human`/`manual` route, and `deterministic` applies only where canonical semantics already permit the deterministic lane (mechanical `verification`, or a `review` role that upstream policy does not require). A `deterministic` sentinel on any role that requires semantic work is refused (`routeApplied=false`) so it can never bypass required review or assurance; canonical policy remains authoritative. Only calibration whose `inventoryFingerprint` matches the freshly recomputed inventory is "current"; a "stale" calibration is shown in explain but never influences the effective profile, route, sentinel, or assurance preference. Generated calibration never deletes or rewrites user overrides, and calibration never lowers canonical task policy or assurance.
