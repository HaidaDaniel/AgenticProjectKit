...
Precedence is deterministic and implemented exactly in `apk execution explain`: built-in default profile -> authored `executionProfile` -> current validated calibration recommendation -> authored `executionOverrides` -> explicit CLI override -> canonical safety/policy validation. A profile is chosen as CLI `--profile` > current calibration profile > authored `executionProfile` > default. A route target is chosen as authored/CLI override > current calibration route (only when the named worker passes capability/availability/capacity/profile checks) > deterministic resolver. Only calibration whose `inventoryFingerprint` matches the freshly recomputed inventory is "current"; a "stale" calibration is shown in explain but never influences the effective route or profile. Generated calibration never deletes or rewrites user overrides, and calibration never lowers canonical task policy or assurance.
