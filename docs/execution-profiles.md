...
Precedence is deterministic and implemented exactly in `apk execution explain`: built-in default profile -> authored `executionProfile` -> current validated calibration recommendation -> authored `executionOverrides` -> explicit CLI override -> canonical safety/policy validation. A profile is chosen as CLI `--profile` > current calibration profile > authored `executionProfile` > default. Route selection distinguishes hard and soft overrides: an explicit resource selection (authored `executionOverrides.resourceId` or CLI `--resource`) outranks the current calibration route, while the soft `preferLocation`/`preferCostClass` preferences only influence deterministic resolver ordering and cannot defeat a calibrated `wait`/`needs-human`. A calibrated worker route applies only when the named worker passes capability/availability/capacity/profile checks. Calibration sentinels are explicit and conservative: a current `wait` produces a calibration-sourced `wait`/`wait` route and a current `needs-human` produces a calibration-sourced `needs-human`/`manual` route, and both may apply even when the underlying canonical lane would be deterministic (for example mechanical `verification`) because they only prevent automatic execution and never weaken policy. A `deterministic` sentinel applies only where canonical semantics already permit the deterministic lane (mechanical `verification`, or a `review` role that upstream policy does not require); on any role that requires semantic work it is refused (`routeApplied=false`) so it can never bypass required review or assurance, and canonical safety/policy (including an unavailable or budget-exhausted assurance requirement) remains authoritative. Only calibration whose `inventoryFingerprint` matches the freshly recomputed inventory is "current"; a "stale" calibration is shown in explain but never influences the effective profile, route, sentinel, or assurance preference. Generated calibration never deletes or rewrites user overrides, and calibration never lowers canonical task policy or assurance.
...
3. Deterministically validates protocol, profile, role IDs, worker IDs, free capacity (`capacity - occupied >= 1`), declared role capability, local-profile/remote-worker conflicts, assurance-level vocabulary, budgets (`maxReviewPasses >= 1`, non-negative integers), and rejects secret-shaped values. Calibration assurance is a raise-only advisory preference: effective assurance is `max(canonical task requirement, calibration preference)`, so calibration never imposes a global floor and never lowers a low/medium policy that allows `none` or `self-check`. Calibration budgets remain informational; the canonical task review budget and completion gate are authoritative and are never weakened into a PASS.
...
An `apk execution explain` view reports the effective profile and its source (`default`/`config`/`calibration`/`cli`), the route and its source (`deterministic`/`calibration`/`override`/`resolver`), assurance (`canonical`, calibration preference, effective), budgets, overrides, and reasons. A current calibration `wait`/`needs-human`/`deterministic` recommendation is reported as an applied calibration-sourced decision (or, for an inapplicable `deterministic` sentinel, as applied=false with the reason that canonical policy is authoritative). JSON also carries the bounded calibration influence (`status`, `planner`, `inventoryFingerprint`, `routeRecommendation`, `routeApplied`, `reason`) and the exact saved recommendation. A stale calibration is explicitly reported and ignored; the per-task completion gate remains authoritative.
...
Task 0087 extends current status and provenance into a bounded CLI attention view. `apk attention [--json]` emits a deterministic, priority-ordered queue derived from existing task, gate, review, policy, and resource state, with task/run, reason, blockers, assurance, review-budget, and next action. `apk workers [--json]` reports declared workers with semantic `ready`/`busy`/`unknown`/`unavailable` state from declared availability/capacity/occupancy plus canonical issued work sessions: an active run for an open task binds the proven task/run and is `busy`, an unactivated/orphaned/malformed session is `unknown` with a bounded diagnostic, a completed run does not hold a slot, and a failure to read canonical session state fails closed to `unknown` (never `ready`). It is a projection over existing canonical records, not a new state store, daemon, poller, or dashboard, and it never claims live process facts.
...
Task 0088 adds optional Git worktrees/isolated workspaces for true parallel top-level mutation. `apk workspaces create` binds task, run, worker/resource, branch, hashed worktree/repository identity, baseline, and candidate revision, and validates repository root, real-path containment, task ownership, and collisions before `git worktree add`. When `--run` is supplied, creation first proves the canonical task/run/resource binding against issued `apk-worker-v1` session records: the session must exist for the exact task and run; an explicit `--resource` must match the canonical `resourceId` exactly; and when `--resource` is omitted the matched session's own canonical `resourceId` is captured automatically onto the record and ownership marker so resource-level provenance is not lost. A missing, malformed, mismatched, or unreadable session is refused before any mutation, and `--resource` without `--run` is refused because the binding could not be re-validated later. A legacy session that genuinely carries no resource identity yields no `resourceId` (no resource is invented) and therefore provides no resource-level provenance. State is `active`/`missing`/`unregistered`/`foreign`/`unsafe`/`unknown`/`ambiguous`; cleanup is dry-run by default and requires `--apply`. Single-worker operation remains unchanged. Cleanup proves exact ownership across the record, Git administrative marker (including `runId`/`resourceId`), Git worktree registration, registered branch, contained real path, worktree identity/hash, repository identity, clean Git state, and the canonical session binding: the record's `resourceId` must still equal the canonical session's `resourceId`, otherwise cleanup fails closed. A bound run is `active` only while its task is open; a done/canceled task makes the activation historical, and an unreadable or malformed session fails closed to `unknown`. APK never deletes user-created, dirty, unmerged, active-run, ambiguous, path-escaping, or repository-root paths.
