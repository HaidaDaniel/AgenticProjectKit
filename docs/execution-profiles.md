...
3. Deterministically validates protocol, profile, role IDs, worker IDs, free capacity (`capacity - occupied >= 1`), declared role capability, local-profile/remote-worker conflicts, assurance-level vocabulary, budgets (`maxReviewPasses >= 1`, non-negative integers), and rejects secret-shaped values. Calibration assurance is an advisory preference only; the canonical task policy/gate clamps effective assurance per task, so calibration never imposes a global floor and never lowers a low/medium policy that allows `none` or `self-check`.
...
Task 0088 adds optional Git worktrees/isolated workspaces for true parallel top-level mutation. `apk workspaces create` binds task, run, worker/resource, branch, hashed worktree/repository identity, baseline, and candidate revision, and validates repository root, real-path containment, task ownership, and collisions before `git worktree add`. State is `active`/`missing`/`unregistered`/`foreign`/`unsafe`/`ambiguous`; cleanup is dry-run by default and requires `--apply`. Single-worker operation remains unchanged. Creation and cleanup fail closed; APK removes only exact APK-owned safe worktrees (record + Git administrative marker + Git registration + contained path + clean Git state all agree) and never deletes user-created, dirty, unmerged, active-run, ambiguous, path-escaping, or repository-root paths.
