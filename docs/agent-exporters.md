...
Use `apk work <task-id> --owner <agent-id> --target <agent> --role <implement|review|fix|verify> --json` to select a role and consume the machine-readable package. Omit `--role` only when APK should resolve the next role from current canonical evidence. Run IDs must remain safe compact path segments. Review packages reference a prepared, immutable review subject; review results must be returned with the issued run ID and role only after the package is activated. Codex and OpenCode exports include the same contract guidance while retaining tool-specific formatting.
