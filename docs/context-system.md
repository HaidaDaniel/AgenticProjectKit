...
- `apk suggest-context` adds deterministic local signals to its lexical fallback: tracked/staged/untracked Git changes, relative JavaScript/TypeScript imports, importers and related tests, task allowed/forbidden patterns, and explicit recent/dependency paths supplied through the API. Unsupported languages or project shapes skip import parsing and retain path/keyword ranking; no compiler, network, model, or embedding service is required. Every returned suggestion includes `role`, `score`, and `reason`. Forbidden paths may be context-only but never edit targets.
