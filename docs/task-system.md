...
`apk workspaces list|status` reports a bounded state (`active`/`missing`/`unregistered`/`foreign`/`unsafe`/`ambiguous`) plus a cleanup/merge/human next action without exposing broad absolute paths. `apk workspaces cleanup <id>` is a dry-run preview by default and requires `--apply`; cleanup proves the record, ownership marker, Git registration, contained real path, task/run binding, and a clean (no unmerged) Git state all agree. It refuses the repository root, path escape, foreign/user or unmarked worktrees, dirty/unmerged work, active-run workspaces, and ambiguous metadata, removing stale metadata only when Git has no registration. `apk status` surfaces bounded non-safe workspace diagnostics and `apk task provenance` lists bounded workspace bindings. APK starts no PTY, SSH, multiplexer, process supervisor, scheduler, automatic merge, or remote executor; an external runtime may open an APK-managed worktree path as a cwd.
