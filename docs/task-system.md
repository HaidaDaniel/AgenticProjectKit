...
- Classification tags add requirements: `migration`, `async`, `worker`, and `security` require independent review; `deployment` and `release` require live evidence; `benchmark` and `evaluation` require benchmark evidence; `provider` and `integration` require report evidence. A category declared only by optional checks is excluded from these evidence requirements; absent categories and categories declared by any required check remain fail-closed.
