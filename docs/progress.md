...
AgenticProjectKit v0.4.1 is prepared for downstream Codex/OpenCode dogfood. It is a patch release over v0.4.0 containing the Task 0106/0107 adoption fixes and the Task 0108 assurance/review correction, installable from the pinned tag `#v0.4.1`. A disposable brownfield adoption smoke test (existing task ids including 0001, a customized `AGENTS.md`, and no `AGENTS.md`) confirms non-destructive adoption, collision-free adoption task ids, canonical `AGENTS.md` generation, preserved customized instructions, current sync, passing lint, expected doctor output, working status and prompts, deterministic verification, and that an ordinary medium task completes without a separate semantic reviewer. No Herdr dependency or runtime orchestration is added.
