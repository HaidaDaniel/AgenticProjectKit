...
1. Define and document the smallest supported Markdown list subset for shared list sections, including indentation, blank lines, nested-list handling, and fenced or code-like content.
2. Replace line-dropping list parsing with deterministic continuation handling while preserving bullet and numbered item normalization and leaving parseSteps semantics unchanged.
3. Decide whether unsupported nested or malformed continuation forms are preserved safely or rejected with an explicit diagnostic; do not silently discard text.
4. Add focused parser, renderer, structured-verification, and claim/writeTaskFile-style mutation regressions for every supported section family.
...
- An indented continuation line belongs to the preceding bullet item and survives parse-render-parse with its semantic text intact.
- A multiline numbered item remains one item and numbered markers continue to be accepted as established by Task 0098.
- Multiline items round-trip in Acceptance criteria, Notes, Required evidence, Context files, allowed and forbidden files, correctness fields, review questions, counterexample searches, Documentation updates, and structured Verification.
- Multiple consecutive multiline items remain separate items with deterministic ordering and no merged or dropped text.
...
- Fenced or code-like content inside a supported list item cannot silently corrupt the task contract; unsupported forms are preserved safely or rejected with a bounded diagnostic.
- Structured verification JSON remains valid and semantically unchanged after render and mutation round-trips.
...
- Focused regression output covering multiline bullets, numbered items, Notes, Required evidence, consecutive items, blank lines, legacy items, Steps isolation, and structured Verification.
- A claim or state-transition-style round-trip regression proving mutation does not delete continuation text.
- Deterministic evidence for the chosen unsupported nested, fenced, and malformed continuation semantics.
- Typecheck, lint, focused tests, full tests, build, dist currency, and git diff checks for the committed candidate.

## Review questions

- Can any non-marker line still disappear from a shared list section during parse or render?
- Can indentation, blank lines, nested markers, or fenced content cause two items to merge or one item to vanish?
- Does the implementation accidentally change Steps ordering or semantics?
- Does structured Verification remain valid when an item spans physical lines?
...
- A bullet item whose continuation is indented by two spaces, four spaces, or a tab.
- Two consecutive multiline bullets with a blank line between them.
