import { resetLocalCommunicationLanguage, resolveCommunicationLanguage, resolveLocalPreferencesPath, writeLocalCommunicationLanguage, } from "../../core/config/index.js";
...
function describeSource(source) {
    if (source === "explicit")
        return "explicit session override";
    if (source === "local")
        return "local preference";
    return "default (English fallback)";
}
export async function runLanguageCommand(argv) {
    if (hasHelpFlag(argv)) {
    ...
    try {
        ...
        throw new Error(`Unknown language subcommand: ${subcommand}. Usage: apk language [show|set <tag>|reset]`);
    }
    catch (error) {
