...
import { createWorkspace, listWorkspaceStatuses, removeWorkspace, renderCreateWorkspaceResult, renderRemoveWorkspaceResult, renderWorkspaceStatuses, } from "../../core/workspaces/index.js";
...
function positionalArgs(argv, valueFlags) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg.startsWith("-"))
            continue;
        if (valueFlags.includes(argv[index - 1] ?? ""))
            continue;
        positional.push(arg);
    }
    return positional;
}
function rejectUnknown(argv, known) {
...
async function runListOrStatus(argv) {
...
export async function runWorkspacesCommand(argv) {
    try {
        ...
        if (subcommand === "create")
            return await runCreate(subArgs);
        if (subcommand === "list" || subcommand === "status")
            return await runListOrStatus(subArgs);
        if (subcommand === "cleanup")
            return await runCleanup(subArgs);
