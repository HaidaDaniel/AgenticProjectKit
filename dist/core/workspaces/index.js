...
function isContainedWithin(parentReal, childReal) {
...
function requireSafeSegment(name) {
...
async function gitError(rootDirectory, args) {
    try {
...
async function resolveRepositoryRoot(rootDirectory) {
    try {
        const result = await git(rootDirectory, ["rev-parse", "--show-toplevel"]);
        const root = result.trim();
        if (!root)
            throw new Error("empty repository root");
        return realpath(root);
    }
    catch (error) {
        throw new WorkspaceSafetyError(`Workspace operations require a Git repository with a resolvable root: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
function findWorktreeEntry(entries, targetReal) {
...
async function readWorkspaceMarker(gitDir) {
    try {
        const parsed = JSON.parse(await readFile(join(gitDir, WORKSPACE_MARKER_FILE), "utf8"));
...
async function worktreeGitDirectory(worktreePath) {
    try {
...
export function workspaceRecordPath(rootDirectory, id) {
...
export async function readWorkspaceRecord(rootDirectory, id) {
    ...
    const raw = parsed;
    if (raw.protocol !== WORKSPACE_PROTOCOL
...
async function readAllWorkspaceRecords(rootDirectory) {
...
function workspaceBaseDirectory(repoRoot, baseDirectory) {
    if (baseDirectory === undefined)
        return join(repoRoot, DEFAULT_WORKSPACE_BASE);
...
async function cleanupCreatedWorktree(repoRoot, worktreePath, branch) {
    try {
...
async function withWorkspaceLock(rootDirectory, taskId, command, run) {
...
async function createWorkspaceLocked(options) {
    ...
    if (task.owner !== options.owner || task.owner === "none") {
        throw new WorkspaceSafetyError(`Task ${task.id} is owned by ${task.owner}, not ${options.owner}; claim it before creating a workspace.`);
    }
    ...
    if (options.runId !== undefined) {
        ...
        // omitted.
        const binding = await resolveCanonicalRunBinding(options.rootDirectory, task.id, options.runId, options.resourceId !== undefined ? { resourceId: options.resourceId } : {});
        if (binding.status !== "matched") {
            throw new WorkspaceSafetyError(`Workspace run binding refused for task ${task.id} run ${options.runId}: ${binding.reason}.`);
        }
    ...
    if (activeForTask.length > 0) {
        throw new WorkspaceSafetyError(`Task ${task.id} already has managed workspace ${activeForTask[0].id}; clean it up before creating another.`);
    }
...
export async function assessWorkspaceSafety(rootDirectory, record, options = {}) {
...
export const WORKSPACE_RUN_STATES = ["active", "terminal", "unknown"];
/**
 ...
 */
export async function assessWorkspaceRun(rootDirectory, taskId, runId, expectedResourceId) {
    const binding = await resolveCanonicalRunBinding(rootDirectory, taskId, runId, expectedResourceId !== undefined ? { resourceId: expectedResourceId } : {});
...
export async function listWorkspaceStatuses(rootDirectory, options = {}) {
    let repoRoot;
    try {
...
export function renderWorkspaceStatuses(entries, json = false) {
    if (json) {
...
export function renderCreateWorkspaceResult(result, json = false) {
    if (json) {
...
export function renderRemoveWorkspaceResult(result, json = false) {
    if (json) {
