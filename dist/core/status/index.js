...
async function localLockWarnings(rootDirectory, taskDirectory) {
    ...
    const locks = [["task", taskLock], ["evidence", evidenceLock]];
    return locks
...
function verificationStatus(gate) {
    return {
...
function provenanceStatus(provenance, gate) {
    if (!provenance) {
        return {
...
function evidenceStatus(provenance) {
    if (!provenance)
        return { total: 0, current: 0, stale: 0, unknown: 0 };
    return {
...
function nextTaskAction(task, gate, dependencies) {
    if (task.state === "todo") {
...
async function summarizeActiveTask(rootDirectory, taskDirectory, file, warnings) {
...
export async function summarizeStatus(rootDirectory) {
...
function renderLatestRun(event) {
    if (!event) {
...
function renderActiveTaskCompact(task) {
...
function renderActiveTaskDetail(task) {
    return [
...
export function renderStatus(summary, options = {}) {
