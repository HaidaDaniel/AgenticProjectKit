...
import { summarizeStatus, } from "./index.js";
export const ATTENTION_PRIORITIES = ["P1", "P2", "P3", "P4"];
export const WORKER_SEMANTIC_STATES = ["ready", "busy", "unknown", "unavailable"];
const PRIORITY_RANK = { P1: 0, P2: 1, P3: 2, P4: 3 };
const TERMINAL_TASK_STATES = ["done", "canceled"];
function bindWorkerSessions(sessions, taskStates) {
    const active = [];
    const stale = [];
    const ambiguous = [];
...
function buildWorkers(registry, sessions, taskStates, diagnostics, sessionsUnavailable = false) {
    ...
    return registry.workers
        ...
        const sortedActive = [...active].sort((left, right) => (left.taskId.localeCompare(right.taskId) || left.runId.localeCompare(right.runId)));
 ...
 */
function sanitizeDiagnostic(warning) {
...
function assuranceStatus(independentReview, review) {
 ...
 */
export async function buildAttentionView(rootDirectory, taskDirectory) {
    ...
    const taskStates = new Map(taskFiles.map((file) => [file.task.id, file.task.state]));
    ...
    const items = summary.activeTasks
    ...
    const diagnostics = [
        ...summary.warnings.map(sanitizeDiagnostic).filter((warning) => warning !== undefined),
        ...workerDiagnostics,
        ...(workers.length === 0 ? ["No declared resource registry; worker occupancy is unavailable."] : []),
    ];
    return { workers, items, diagnostics };
}
export function renderAttentionView(view, json = false) {
    if (json) {
...
function renderWorkerLine(label, worker) {
...
export function renderWorkersView(view, json = false) {
    if (json) {
