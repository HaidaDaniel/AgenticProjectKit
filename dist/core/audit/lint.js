...
function lintTaskStateOwner(task, registeredAgents, agentRegistryAvailable, findings, path) {
...
const ALWAYS_LOADED_INSTRUCTION_FILES = ["AGENTS.md", "CLAUDE.md", "GEMINI.md"];
...
async function fileExists(rootDirectory, path) {
    try {
...
async function estimateFileUnits(rootDirectory, path, unavailable) {
    try {
...
function normalizedCommandLine(line) {
    ...
    if (trimmed.length === 0 || !COMMAND_LINE.test(trimmed))
        return undefined;
 ...
 * a planned output.
 */
async function evaluateContextHygiene(rootDirectory, taskDirectory, documents, findings) {
    const unavailable = new Set();
    const sizes = new Map();
    const sizeOf = async (path) => {
        if (!sizes.has(path))
            sizes.set(path, await estimateFileUnits(rootDirectory, path, unavailable));
        return sizes.get(path) ?? 0;
    };
    const instructionSet = new Set();
    for (const path of ALWAYS_LOADED_INSTRUCTION_FILES) {
        if (await fileExists(rootDirectory, path))
            instructionSet.add(path);
    }
    const taskContractSet = new Set(documents.active.map((file) => normalizeRepoPath(relative(rootDirectory, file.path))));
...
export function renderTaskLintResult(result, json = false) {
    ...
    return [
        ...
        ...result.contextHygiene.estimates.map((estimate) => `  - ${estimate.category}: ${estimate.units} units / ${estimate.files.length} file(s)`),
