...
async function defaultProcessStartIdentity(pid) {
    if (pid === process.pid)
...
async function missingOrRead(path, readLockFile = (target) => readFile(target, "utf8")) {
    for (let attempt = 1; attempt <= LOCK_READ_MAX_ATTEMPTS; attempt += 1) {
        try {
            ...
            if (code === "ENOENT")
                return undefined;
            if (!TRANSIENT_LOCK_READ_ERRORS.has(code ?? "") || attempt === LOCK_READ_MAX_ATTEMPTS)
                throw error;
...
export async function inspectLocalLock(path, runtime = {}) {
    const values = runtimeValues(runtime);
    const raw = await missingOrRead(path, values.readLockFile);
    if (raw === undefined)
        return { path, state: "absent", reason: "lock does not exist", old: false };
    let parsed;
    try {
    ...
    if (liveness === "dead")
        return deadInspection(path, parsed, ageMs, old);
...
export async function inspectLocalMutationLock(path, runtime = {}) {
...
export function renderLocalLockInspection(inspection) {
...
function lockMetadata(options, values, kind = options.kind) {
    return {
        schema: 1,
...
async function tryPublishLock(path, metadata, values) {
...
async function removeIfOwned(path, metadata, values, invokeHook) {
    if (!sameOwner(parseMetadata(await missingOrRead(path, values.readLockFile)), metadata))
        return false;
    if (invokeHook)
        await values.beforeOwnedRemoval?.(path, metadata);
    if (!sameOwner(parseMetadata(await missingOrRead(path, values.readLockFile)), metadata))
        return false;
    await rm(path, { force: true });
    return true;
}
function lockError(options, inspection) {
...
async function withRecoveryGuard(options, run) {
...
export async function withLocalMutationLock(options, run) {
    ...
    while (!await tryPublishLock(options.path, metadata, values)) {
        ...
        if (recovery.state !== "absent") {
            ...
            if (recovery.state !== "live" || values.now() >= deadline)
                throw lockError(options, recovery);
            await wait(retryMs);
            continue;
        }
...
export async function recoverLocalLock(options) {
    ...
    const inspection = await inspectLocalLock(options.path, options.runtime);
    if (inspection.state === "absent")
        return { recovered: recoveredMarker, inspection };
