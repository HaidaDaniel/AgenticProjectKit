...
function validateReviewRunId(value) {
    if (!isSafeRunId(value)) {
        throw new Error("Review run id must be a compact identifier.");
    }
    return value;
}
async function writePreparedReview(rootDirectory, preparation) {
...
async function readPreparedReview(rootDirectory, taskId, id) {
    ...
    return {
        ...
        subject: raw.subject,
        changedFiles: raw.changedFiles.filter((path) => typeof path === "string"),
...
function requireReviewableTask(task, reviewer) {
...
export function renderTaskReviewPrompt(input) {
...
export async function prepareTaskReview(options) {
...
function asTaskReviewRecord(record) {
    if (record.type !== "review" ||
        !TASK_REVIEW_OUTCOMES.includes(record.result) ||
        !record.reviewer) {
        return undefined;
    }
    return {
...
export async function recordTaskReview(options) {
    ...
    if (options.reviewRunId) {
        ...
        if (stored.origin === "worker") {
            ...
            const activeSession = await readActiveWorkerSession(options.rootDirectory, task.id, workerRunId);
            if (activeSession.runId !== stored.reviewRunId
...
export async function listTaskReviews(rootDirectory, taskId) {
    const records = await readTaskEvidence(rootDirectory, taskId);
    return records
        .map(asTaskReviewRecord)
        .filter((record) => record !== undefined);
}
export function assessTaskReview(record, currentSubject) {
...
export function assessTaskReviews(records, currentSubject) {
...
export async function recordTaskHumanDecision(options) {
    ...
    if (actor === recorderAgent.id) {
        throw new Error("Operator actor must be distinct from the recording agent; an agent cannot authorize itself.");
    }
...
function asTaskHumanDecisionRecord(record) {
    if (record.type !== "human-decision" || !TASK_HUMAN_DECISIONS.includes(record.decision)) {
        return undefined;
    }
    return record;
}
export async function listTaskHumanDecisions(rootDirectory, taskId) {
    const records = await readTaskEvidence(rootDirectory, taskId);
    return records
        .map(asTaskHumanDecisionRecord)
        .filter((record) => record !== undefined);
}
export function assessTaskHumanDecision(record, currentSubject) {
    const freshness = compareTaskEvidenceFreshness(record, currentSubject);
    return {
        record,
        freshness: freshness.freshness,
...
export function assessTaskHumanDecisions(records, currentSubject) {
    return records.map((record) => assessTaskHumanDecision(record, currentSubject));
}
export function renderTaskHumanDecisionResult(record) {
    return [
        `Task: ${record.taskId}`,
