...
export const TASK_EVIDENCE_REPOSITORIES = ["git", "none"];
...
function textValue(value, label, issues, maxLength = 160) {
...
function optionalText(value, label, issues, maxLength = 240) {
    if (value === undefined) {
        return undefined;
    }
    return textValue(value, label, issues, maxLength);
}
function optionalTextList(value, label, issues, maxItems = 32, maxItemLength = 320) {
    if (value === undefined) {
        return undefined;
    }
...
function optionalBoolean(value, label, issues) {
    if (value === undefined)
        return undefined;
...
function optionalNonNegativeInteger(value, label, issues, max = 1_000_000) {
    if (value === undefined) {
        return undefined;
    }
...
function optionalMetrics(value, label, issues) {
    if (value === undefined) {
        return undefined;
    }
    if (!isRecord(value)) {
...
function oneOf(value, allowed, label, issues) {
...
function validateSubject(value, label, issues) {
    if (!isRecord(value)) {
    ...
    const repository = oneOf(value.repository, TASK_EVIDENCE_REPOSITORIES, `${label}.repository`, issues);
    const subject = {
...
function normalizeEvidenceRecord(value, lineNumber) {
    ...
    const subjectRunId = textValue(value.subject && isRecord(value.subject) ? value.subject.runId : undefined, `${prefix}.subject.runId`, issues, 120);
    const record = {
...
async function withEvidenceAppendLock(rootDirectory, run) {
...
export async function appendTaskEvidence(rootDirectory, input, uniqueness) {
...
export async function readTaskEvidence(rootDirectory, taskId) {
    const path = join(rootDirectory, TASK_EVIDENCE_PATH);
    let content;
    try {
...
export function filterTaskEvidence(records, taskId, result) {
    return records.filter((record) => (record.taskId === taskId && (result === undefined || record.result === result)));
}
export function compareTaskEvidenceFreshness(record, current) {
    const issues = [];
...
export function renderTaskEvidence(records, taskId) {
