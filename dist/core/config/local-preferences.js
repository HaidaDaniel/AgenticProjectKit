 ...
 */
export function resolveLocalPreferencesPath(environment = {}) {
    ...
    if (platform === "darwin") {
        return join(home, "Library", "Application Support", LOCAL_PREFERENCES_DIRECTORY, LOCAL_PREFERENCES_FILE);
    }
...
async function readRawPreferences(path) {
    try {
...
export async function readLocalPreferences(path) {
...
export async function writeLocalCommunicationLanguage(language, path) {
    ...
    await writeFile(resolved, `${JSON.stringify({ ...raw, communicationLanguage: normalized }, null, 2)}\n`, "utf8");
    return normalized;
}
export async function resetLocalCommunicationLanguage(path) {
    const resolved = path ?? resolveLocalPreferencesPath();
    try {
        await stat(resolved);
    }
