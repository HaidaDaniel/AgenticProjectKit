import { RESOURCE_COST_CLASSES, RESOURCE_LOCATIONS, } from "../resources/index.js";
export const EXECUTION_PROFILES = ["local", "constrained", "balanced", "abundant"];
...
export const EXECUTION_COMPLEXITIES = ["simple", "medium", "complex"];
export const EXECUTION_PROFILE_SOURCES = ["default", "config", "calibration", "cli"];
export const EXECUTION_ROUTE_SOURCES = ["deterministic", "calibration", "override", "resolver"];
export const CALIBRATION_ROUTE_SENTINELS = ["deterministic", "wait", "needs-human"];
...
const DEFAULT_PROFILE = "constrained";
const DEFAULT_ASSURANCE_BUDGET = {
    maxReviewPasses: 2,
...
function oneOf(value, values, label, issues) {
...
export function parseExecutionProfile(value) {
    const profile = oneOf(value, EXECUTION_PROFILES, "executionProfile", []);
    if (!profile)
        throw new ExecutionValidationError([`executionProfile must be one of: ${EXECUTION_PROFILES.join(", ")}.`]);
    return profile;
}
export function parseExecutionOverride(value) {
    if (!isRecord(value))
        throw new ExecutionValidationError(["executionOverrides must be an object."]);
    const issues = [];
    ...
    if (issues.length > 0)
        throw new ExecutionValidationError(issues);
    return {
...
export function resolveAssurancePlan(request) {
    ...
    const reviewWorkers = request.registry.workers.filter((worker) => {
        if (!supportsRole(worker, "review") || worker.availability !== "available" || worker.occupied >= worker.capacity)
            return false;
...
function supportsRole(worker, role) {
    const aliases = ROLE_ALIASES[role];
    return worker.capabilities.roles.some((candidate) => aliases.includes(candidate));
}
function profileAllows(profile, worker, request, complexity) {
    const { role } = request;
    if (profile === "local")
        return worker.location === "local";
...
function candidateReasons(worker, request, profile, complexity) {
    const reasons = [];
    if (!supportsRole(worker, request.role))
        reasons.push(`role ${request.role} is not declared`);
    if (worker.availability !== "available")
        reasons.push(`availability=${worker.availability}`);
    if (worker.occupied >= worker.capacity)
    ...
    if (request.contextLimit !== undefined) {
        ...
        if (limit !== undefined && limit < request.contextLimit)
            reasons.push(`context limit ${limit} is below ${request.contextLimit}`);
    }
    if (request.requiredTools) {
        for (const tool of request.requiredTools) {
            if (!worker.capabilities.tools.includes(tool))
...
function calibrationInfluence(request, routeApplied, reason) {
    if (!request.calibration)
        return {};
    return { calibration: { ...request.calibration, routeApplied, reason } };
}
function deterministicRoute(request, profile, profileSource) {
...
export function resolveExecutionRoute(request) {
    ...
    const calibrationWorkerRoute = calibrationRoute !== undefined
        && !CALIBRATION_ROUTE_SENTINELS.includes(calibrationRoute)
        ? calibrationRoute
        : undefined;
    ...
    const sentinelRoute = (kind, queue, explanation, reason) => ({
    ...
    if (!override?.resourceId && calibrationSentinel === "wait") {
        return sentinelRoute("wait", "wait", "Current calibration recommends waiting; no worker is dispatched.", "current calibration sentinel: wait");
    }
    if (!override?.resourceId && calibrationSentinel === "needs-human") {
        return sentinelRoute("needs-human", "manual", "Current calibration recommends a human decision; the resolver does not auto-select a worker.", "current calibration sentinel: needs-human");
    }
    // No sentinel applies: the canonical deterministic lane wins where defined.
    if (deterministic)
        return deterministic;
    let selected;
    let routeSource = "resolver";
    let explanation;
...
export function renderExecutionRoute(route, json = false) {
    if (json)
    ...
    lines.push("Candidates:", ...route.candidates.map((candidate) => `- ${candidate.resourceId}: ${candidate.eligible ? "eligible" : "rejected"}; ${candidate.reasons.join(", ")}`));
    return `${lines.join("\n")}\n`;
}
