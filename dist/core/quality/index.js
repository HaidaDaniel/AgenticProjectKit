...
async function findGoTestFiles(rootDirectory) {
    ...
    const walk = async (relative, depth) => {
        visited += 1;
        if (visited > MAX_GO_TEST_DISCOVERY_DIRS)
            return;
        let entries;
        try {
...
async function detectMarkers(rootDirectory, states) {
    ...
    if (await exists(join(rootDirectory, "go.mod"))) {
        ...
        if (goTestFiles.length > 0) {
            addMarkerEvidence(states, "tests", `${goTestFiles.length} *_test.go file(s)`, `Go package-local test files detected: ${goTestFiles.slice(0, 3).join(", ")}`, "high");
        }
    }
