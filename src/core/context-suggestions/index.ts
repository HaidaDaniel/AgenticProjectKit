...
async function importedDependencyMap(
  ...
  for (const file of files) {
    if (!/\.(c|m)?tsx?$|\.(c|m)?jsx?$/.test(file)) continue;
    let content: string;
    try {
    ...
    for (const specifier of importSpecifiers(content)) {
      ...
      if (!target) continue;
      const importers = dependencies.get(target) ?? new Set<string>();
      importers.add(file);
...
function lexicalScore(path: string, tokens: readonly string[]): number {
...
function taskPathAllowed(path: string, task: ProjectTask | undefined): boolean {
  return task ? task.allowedFiles.some((pattern) => pathMatches(path, pattern)) : true;
}

function taskPathForbidden(path: string, task: ProjectTask | undefined, extra: readonly string[]): boolean {
  return [
    ...(task?.forbiddenFiles ?? []),
    ...extra,
...
function relatedToChangedTest(path: string, changed: ReadonlySet<string>): boolean {
...
function suggestionRole(path: string, task: ProjectTask | undefined, forbidden: boolean): "context" | "implementation" {
  if (forbidden || !taskPathAllowed(path, task)) return "context";
  if (task) return "implementation";
  return isTestPath(path) || !path.toLowerCase().endsWith(".md") ? "implementation" : "context";
}

function suggestionReason(reasons: readonly string[], role: "context" | "implementation"): string {
  const unique = [...new Set(reasons)];
  if (unique.length === 0) unique.push("lexical task/path match");
  if (role === "context" && !unique.includes("context-only scope")) unique.push("context-only scope");
  return unique.join(", ");
}
...
export async function suggestContext(
  rootDirectory: string,
  description: string,
  options: ContextSuggestionOptions = {},
  ...
  for (const changedPath of changed) {
    for (const importer of importedBy.get(changedPath) ?? []) changedDependencies.add(importer);
  }
  const changedImports = new Set<string>();
  for (const changedPath of changed) {
    ...
    for (const specifier of importSpecifiers(content)) {
      ...
      if (target) changedImports.add(target);
    }
  }
