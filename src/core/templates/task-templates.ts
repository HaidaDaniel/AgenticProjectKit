...
const templates: Record<TaskTemplateType, TaskTemplateDefaults> = {
  ...
  bugfix: {
    ...
    steps: [
      "Capture a failing signal first: reproduce the old behavior and record the exact failing observation before changing implementation.",
      "Minimize the reproducer when it is economical; otherwise record why full reproduction is impractical and what was attempted.",
      "List competing hypotheses and let evidence from the failing signal choose among them; do not treat the first hypothesis as proven root cause.",
      "Add bounded instrumentation only where it distinguishes hypotheses, then remove or justify it.",
      "Implement the smallest safe fix; do not refactor unrelated code.",
      "Show the failing signal now passes, or state the explicit best-effort limit, and add practical regression protection.",
    ],
    acceptanceCriteria: [
    ...
    documentationUpdates: docsUpdate,
    notes: ["Keep the fix narrow. Reproduction is best-effort, not a forced completion gate."],
  },
  refactor: {
    type: "refactor",
    mode: "product",
  ...
  release: {
    ...
    tags: ["release"],
    contextFiles: ["AGENTS.md", "docs/architecture.md", "docs/task-system.md"],
    verification: [automated("release-report", { profile: "report", artifact: "reports/release.json" }), live("release-smoke", "Observe the exact-SHA hosted CI result, annotated tag peel, fresh-store actual-tag cold install, package version/bin/alias/portable assets, and a representative downstream flow against the immutable released SHA.", "release URL, CI run, tag object, or release id")],
    steps: [
      "Resolve the free next SemVer target and confirm existing tags are immutable; never move or rewrite a published tag.",
      "Freeze the exact candidate SHA/tree and observe every declared PRE-TAG criterion against that exact SHA, including local quality/coverage/build/release checks, committed dist currency, package/bin/asset payload, declared compatibility or downstream smoke, and exact-SHA hosted CI.",
      "Create the annotated tag only after all PRE-TAG criteria pass, pointing to the validated candidate SHA, and verify the peeled commit equals that SHA.",
      "Run POST-TAG checks separately: actual immutable-tag cold install, tag-peel verification, released package/bin identity, downstream install from the tag, and post-release self-adoption.",
      "Record post-tag evidence in a post-release artifact, never backfilled into the release-note file committed inside the tag, and do not mutate the candidate or tag after evidence is captured.",
    ],
    acceptanceCriteria: [
    ...
    requiredEvidence: ["Pre-tag criteria observed against the frozen candidate SHA, exact-SHA hosted CI, annotated tag peel to that SHA, and a distinct post-tag cold-install/released-consumer record."],
    reviewQuestions: [
    ...
    documentationUpdates: docsUpdate,
    notes: ["Treat pre-tag evidence as the gate for tag creation and post-tag evidence as a separate record; never fabricate future facts inside an immutable commit."],
  },
};
